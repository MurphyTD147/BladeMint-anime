...
function Header({ experience, searchQuery, setSearchQuery, onSelectAnime }) {
  ...
  return (
    <header className="max-w-7xl mx-auto py-3 md:py-6 px-4 md:px-6 relative text-white z-50">
      {/* ОСНОВНОЙ КОНТЕЙНЕР: на мобилках колонкой, на десктопе в ряд */}
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
        
        {/* ВЕРХНЯЯ ПАНЕЛЬ: Логотип + Кнопки + Профиль (Всегда в одну строку) */}
        <div className="flex justify-between items-center w-full md:w-auto gap-4">
          ...
          {/* КНОПКИ И СТАТЫ (только для мобилок они прижимаются вправо) */}
          <div className="flex items-center gap-2 md:hidden">
            ...
            {/* Опыт (мини-версия) */}
            <div className="bg-white/[0.03] border border-white/5 px-2 py-1.5 rounded-md flex items-center gap-2">
        ...
        {/* СТРОКА ПОИСКА: На мобилках идет вторым рядом */}
        <div className={`relative w-full md:max-w-md transition-all duration-500 ${searchActive ? 'md:flex-grow' : 'md:w-64'}`}>
          ...
          {searchActive && searchResults.length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-[#0d0d0d] border border-white/10 rounded-md overflow-hidden shadow-2xl z-[60]">
              {searchResults.map((anime) => (
                ...
                >
                  ...
                  <div className="flex flex-col">
                    <span className="text-[9px] font-bold uppercase tracking-wider">{anime.title}</span>
                    <span className="text-[7px] text-knight-steel uppercase opacity-60 truncate">{anime.originalTitle}</span>
                  </div>
                </div>
              ))}
            </div>
